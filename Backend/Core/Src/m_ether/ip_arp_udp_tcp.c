...
void make_udp_reply_from_request(uint8_t *buf, char *data, uint16_t datalen,
		uint16_t port) {
	uint8_t i = 0;
	uint16_t ck;
	make_eth(buf);
...
// packetloop_icmp_tcp(buf,ENC28J60_PacketReceive(BUFFER_SIZE, buf));
...
void send_udp_prepare(uint8_t *buf, uint16_t sport, uint8_t *dip,
		uint16_t dport) {
	uint8_t i = 0;
	while (i < 6) {
		buf[ETH_DST_MAC + i] = gwmacaddr[i]; // gw mac in local lan or host mac
...
void send_udp(uint8_t *buf, char *data, uint16_t datalen, uint16_t sport,
...
void send_wol(uint8_t *buf,uint8_t *wolmac)
{
	...
	ENC28J60_PacketSend(UDP_HEADER_LEN+IP_HEADER_LEN+ETH_HEADER_LEN+102,buf);
}
#endif // WOL_CLIENT
...
uint16_t build_tcp_data(uint8_t *buf, uint16_t srcPort) {
	...
	buf[TCP_CHECKSUM_L_P] = ck & 0xff;
	// 4 is the tcp mss option:
//        ENC28J60_PacketSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+ETH_HEADER_LEN+4,buf);
	...
	if (statuscode == 0 && len_of_data > 12) {
		...
		if (client_browser_callback) {
			if (strncmp("200", (char *) &(bufptr[datapos + 9]), 3) == 0) {
				(*client_browser_callback)(0,
						((uint16_t) TCP_SRC_PORT_H_P
								+ (bufptr[TCP_HEADER_LEN_P] >> 4) * 4),
						len_of_data);
			} else {
				(*client_browser_callback)(1,
						((uint16_t) TCP_SRC_PORT_H_P
								+ (bufptr[TCP_HEADER_LEN_P] >> 4) * 4),
						len_of_data);
			}
...
//
//
void client_browse_url(char *urlbuf, char *urlbuf_varpart, char *hoststr,
...
void client_http_post(char *urlbuf, char *hoststr, char *additionalheaderline,
		char *method, char *postval, void (*callback)(uint8_t, uint16_t, uint16_t)) {
	client_urlbuf = urlbuf;
	client_hoststr = hoststr;
